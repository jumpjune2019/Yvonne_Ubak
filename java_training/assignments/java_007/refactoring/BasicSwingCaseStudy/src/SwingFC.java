...
class SwingFC implements ActionListener {
	...
	WindowFunc setFrame = (jfrm, title, w, h, tf, first,second, btn, lblF, lblS) -> {
		// Create a new JFrame container.
				...
				jtfFirst = new JTextField(tf);
				jtfSecond = new JTextField(tf);
				// Set the action commands for the text fields.
				jtfFirst.setActionCommand(first);
				jtfSecond.setActionCommand(second);
				// Create the Compare button.
				JButton jbtnComp = new JButton(btn);
				// Add action listener for the Compare button.
				jbtnComp.addActionListener(this);
				// Create the labels.
				jlabFirst = new JLabel(lblF);
				jlabSecond = new JLabel(lblS);
	...
	SwingFC() {
		setFrame.app(frm, "Compare Files", 200, 190, 14, "fileA", "fileB", 
